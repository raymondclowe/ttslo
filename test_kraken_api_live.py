...
# Map .env keys to expected test environment variables (only if not already set)
if not os.environ.get('COPILOT_W_KR_RW_PUBLIC'):
    os.environ['COPILOT_W_KR_RW_PUBLIC'] = os.environ.get('KRAKEN_API_KEY_RW', '')
if not os.environ.get('COPILOT_W_KR_RW_SECRET'):
...
class TestKrakenAPILive:
    ...
    def test_01_live_add_query_modify_cancel_limit_order(
        ...
        try:
            ...
            # Check volume is close (Kraken may round)
            returned_volume = float(order['vol'])
            assert abs(returned_volume - volume) < 0.00001, \
                f"Order volume {returned_volume} should be close to {volume}"
    ...
    def test_04_live_buy_limit_order_lifecycle(
        self, live_api, test_logger, btc_usdt_current_price
    ):
        """
        Test buy order lifecycle: add limit order -> query -> modify -> query -> cancel -> query.
        ...
        Uses BTC/USDT pair since account has USDT balance.
        """
        # Step 1: Add limit buy order at unreasonable price
        volume = self.get_next_volume(6)  # Continue from previous tests
        buy_price = self.get_unreasonable_buy_price(btc_usdt_current_price)
        
        logger.info(f"\n{'#'*80}")
        logger.info(f"TEST 4: BUY ORDER - Add -> Query -> Modify -> Query -> Cancel -> Query")
        logger.info(f"Volume: {volume} BTC")
        logger.info(f"Buy Price: ${buy_price:,.2f} (current: ${btc_usdt_current_price:,.2f})")
        logger.info(f"{'#'*80}\n")
        ...
        try:
            ...
            assert 'txid' in add_result, "Order should return txid"
            txid = add_result['txid'][0]
            logger.info(f"✓ Buy order created: {txid}")
            
        except Exception as e:
            test_logger.log_operation('add_order', add_params, None, error=e)
            pytest.fail(f"Failed to add buy order: {e}")
        ...
        try:
            ...
            order = query_result['open'][txid]
            assert order['descr']['type'] == 'buy', "Order should be buy type"
            # Check volume is close (Kraken may round)
            returned_volume = float(order['vol'])
            assert abs(returned_volume - volume) < 0.00001, \
                f"Order volume {returned_volume} should be close to {volume}"
            logger.info(f"✓ Buy order verified in open orders")
        ...
        # Step 3: Modify the order (increase volume slightly)
        new_volume = self.get_next_volume(7)
        modify_params = {
            'txid': txid,
            'pair': 'BTC/USDT',
            'volume': new_volume,
        ...
        try:
            ...
            if 'txid' in modify_result:
                new_txid = modify_result['txid']
                logger.info(f"✓ Buy order modified, new txid: {new_txid}")
                txid = new_txid
            else:
                logger.info(f"✓ Buy order modified: {txid}")
            
        except Exception as e:
            test_logger.log_operation('edit_order', modify_params, None, error=e)
            logger.warning(f"Failed to modify buy order (may not be supported): {e}")
        ...
        try:
            ...
            if txid in query_result2.get('open', {}):
                order = query_result2['open'][txid]
                logger.info(f"✓ Buy order still in open orders, volume: {order['vol']}")
            else:
                logger.info(f"✓ Order {txid} not found (may have been replaced)")
        ...
        try:
            ...
            logger.info(f"✓ Buy order cancelled: {txid}")
            
        except Exception as e:
            test_logger.log_operation('cancel_order', cancel_params, None, error=e)
            pytest.fail(f"Failed to cancel buy order: {e}")
        ...
        try:
            ...
            assert txid not in query_result3.get('open', {}), \
                f"Order {txid} should not be in open orders after cancellation"
            logger.info(f"✓ Buy order confirmed cancelled and removed from open orders")
            
        except Exception as e:
            test_logger.log_operation('query_open_orders', {}, None, error=e)
            pytest.fail(f"Failed to query orders after cancel: {e}")
        
        logger.info(f"\n✓✓✓ TEST 4 PASSED: Buy order lifecycle completed successfully\n")
    
    def test_05_live_buy_trailing_stop_and_cancel(
        self, live_api, test_logger, btc_usdt_current_price
    ):
        """
        Test buy trailing stop order: add -> query -> cancel -> query.
        
        This test:
        1. Creates a trailing stop buy order with 10% offset (won't execute)
        2. Queries to verify order was created
        3. Cancels the order
        4. Queries to verify cancellation
        ...
        Uses BTC/USDT pair since account has USDT balance.
        """
        volume = self.get_next_volume(8)
        
        logger.info(f"\n{'#'*80}")
        logger.info(f"TEST 5: BUY TRAILING STOP - Add -> Query -> Cancel -> Query")
        logger.info(f"Volume: {volume} BTC")
        logger.info(f"Trailing offset: 10%")
        logger.info(f"{'#'*80}\n")
        
        # Step 1: Add trailing stop buy order
        add_params = {
            'pair': 'BTC/USDT',
            'direction': 'buy',
            'volume': volume,
        ...
        try:
            ...
            assert 'txid' in add_result, "Order should return txid"
            txid = add_result['txid'][0]
            logger.info(f"✓ Buy trailing stop order created: {txid}")
            
        except Exception as e:
            test_logger.log_operation('add_trailing_stop_loss', add_params, None, error=e)
            pytest.fail(f"Failed to add buy trailing stop order: {e}")
        ...
        try:
            ...
            assert order['descr']['ordertype'] == 'trailing-stop', \
                "Order should be trailing-stop type"
            logger.info(f"✓ Buy trailing stop order verified in open orders")
        ...
        try:
            ...
            logger.info(f"✓ Buy trailing stop order cancelled: {txid}")
            
        except Exception as e:
            test_logger.log_operation('cancel_order', cancel_params, None, error=e)
            pytest.fail(f"Failed to cancel buy trailing stop order: {e}")
        ...
        try:
            ...
            assert txid not in query_result2.get('open', {}), \
                f"Order {txid} should not be in open orders after cancellation"
            logger.info(f"✓ Buy trailing stop order confirmed cancelled")
            
        except Exception as e:
            test_logger.log_operation('query_open_orders', {}, None, error=e)
            pytest.fail(f"Failed to query orders after cancel: {e}")
        
        logger.info(f"\n✓✓✓ TEST 5 PASSED: Buy trailing stop lifecycle completed successfully\n")
    
    def test_06_live_multiple_buy_orders_batch_cancel(
        self, live_api, test_logger, btc_usdt_current_price
    ):
        """
        Test creating multiple buy orders and cancelling them.
        
        This test:
        1. Creates 3 buy orders with different volumes
        2. Queries to verify all were created
        3. Cancels all orders
        4. Queries to verify all were cancelled
        ...
        """
        logger.info(f"\n{'#'*80}")
        logger.info(f"TEST 6: BUY ORDERS - Create Multiple Orders -> Query -> Cancel All -> Query")
        logger.info(f"{'#'*80}\n")
        
        txids = []
        buy_price = self.get_unreasonable_buy_price(btc_usdt_current_price)
        
        # Step 1: Create 3 buy orders with different volumes
        for i in range(9, 12):  # iterations 9, 10, 11
            volume = self.get_next_volume(i)
            add_params = {
                'pair': 'BTC/USDT',
                'order_type': 'limit',
            ...
            try:
                ...
                txid = add_result['txid'][0]
                txids.append(txid)
                logger.info(f"✓ Buy order {i-8}/3 created: {txid} (volume: {volume})")
                
            except Exception as e:
                test_logger.log_operation('add_order', add_params, None, error=e)
                logger.error(f"Failed to create buy order {i-8}/3: {e}")
        
        if not txids:
            pytest.fail("Failed to create any buy orders")
        ...
        try:
            ...
            logger.info(f"✓ All {len(txids)} buy orders verified in open orders")
        ...
        for i, txid in enumerate(txids, 1):
            ...
            try:
                ...
                logger.info(f"✓ Buy order {i}/{len(txids)} cancelled: {txid}")
                
            except Exception as e:
                test_logger.log_operation('cancel_order', cancel_params, None, error=e)
                logger.error(f"Failed to cancel buy order {txid}: {e}")
        ...
        try:
            ...
            logger.info(f"✓ All {len(txids)} buy orders confirmed cancelled")
            
        except Exception as e:
            test_logger.log_operation('query_open_orders', {}, None, error=e)
            pytest.fail(f"Failed to query orders after cancellations: {e}")
        
        logger.info(f"\n✓✓✓ TEST 6 PASSED: Multiple buy orders lifecycle completed successfully\n")
